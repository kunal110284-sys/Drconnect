-- Migration: 20260913220000_consultation_otp_verification.sql
-- Enables patients to share a 4-digit consultation passcode and doctors to verify
-- the passcode at consultation end, securely transitioning slots to 'completed'
-- across both doctor_appointments and care_requests under RLS.

-- 1. Function: ensure_consultation_passcode
...
GRANT EXECUTE ON FUNCTION public.ensure_consultation_passcode(UUID, TEXT) TO authenticated;


-- 2. Function: verify_consultation_otp
...
GRANT EXECUTE ON FUNCTION public.verify_consultation_otp(UUID, TEXT, TEXT) TO authenticated;
